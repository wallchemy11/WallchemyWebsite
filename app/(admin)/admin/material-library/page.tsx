...
export default function MaterialLibraryPage() {
  ...
  async function loadItems() {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/data?page=material-library-list");
      if (res.status === 401) {
  ...
  async function saveItem() {
    try {
      const res = await fetch("/api/admin/material-library", {
        method: "POST",
      ...
      console.error("Error saving material library item:", error);
    }
  }
  ...
  async function uploadMedia(file: File) {
    ...
    try {
      ...
      body.append("folder", "material-library");
  ...
  async function deleteItem(id: number) {
    if (!confirm("Delete this material library item?")) return;
    try {
      const res = await fetch(`/api/admin/material-library?id=${id}`, { method: "DELETE" });
      if (res.ok) {
        loadItems();
      }
    } catch (error) {
    ...
    <div className="text-alabaster">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold sm:text-3xl">Material Library Management</h1>
        ...
        {editing !== null && (
          <div className="mb-8 rounded-lg border border-alabaster/10 bg-alabaster/5 p-5 sm:p-6">
            <h2 className="mb-4 text-xl font-semibold">
              {editing === -1 ? "New Material Library Item" : "Edit Material Library Item"}
            </h2>
        ...
        <div className="space-y-4">
          ...
          {items.length === 0 ? (
            <p className="text-sm text-alabaster/60">No material library items yet.</p>
          ) : null}
        </div>
      </div>
    </div>
